...
from keras.layers import Embedding, Input, Dense, merge, concatenate, Flatten, Dropout, Permute, Reshape, Lambda
...
def get_model(num_users, num_items, layers=[20, 10], reg_layers=[0, 0]):
    ...
    vector = Permute((2, 1))(vector)
    print('1:',vector.shape)
    vector = Reshape((4,15,100))(vector)
    vector = Permute((2, 1,3))(vector)
    vector = Reshape((15, 400))(vector)
    print('2:',vector.shape)
    vector = GlobalAveragePooling1D()(vector)
    print('3:', vector.shape)
    vector = Reshape((4, 100))(vector)
    #vector = mean(vector,axis=1)
    print('4',vector.shape)
    O_seq = Attention(5, 120)([vector, vector, vector])
    print('5',O_seq.shape)
...
if __name__ == '__main__':
    vocabulary_lenth = 53110
    #vocabulary_lenth = 50100
    #vocabulary_lenth = 48000
    with open("user_hist_withinfo_music_HK", "rb") as file:
        user_info = pickle.load(file)

    with open("song_info_music_HK_w", "rb") as file:
    ...
    print("End. Best Iteration %d:  H3: %.4f, H5: %.4f, N3: %.4f, N5: %.4f, A3: %.4f, A5: %.4f, H1: %.4f, RR: %.4f. " % (
        best_iter, best_hr, best_h5, best_ndcg, best_n5, best_ap, best_a5, best_h1, best_rr))
    if args.out > 0:
        print("The best MLP model is saved to %s" % (model_out_file))
    file_name = "KSAMLP.txt"
