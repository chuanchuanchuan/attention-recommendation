...
model.load_weights('music/Pretrain/savedmodel')
w = model.get_weights()
...
for uu in range(3000):
    ...
    for s in song_new:
        ...
        vector = w[0][user_input + item_input]  # 15,100
        vector = vector.transpose((1,0))
        vector = vector.reshape((4,1500))
        vector = vector.mean(axis=0)
        vector = vector.reshape((15,100))
        cl=[]
        for i in range(5):
            QQ = w[1]  # 100,600
